...
int main (int argc, char** argv) {
    ...
    */
    ...
    while (1) {
	    ...
	    else if (add_daemon == true and get_active_threads() >= MAX_CLIENT_DAEMONS){
	...
	memset(buffer, '\0', reply.length());//TODO: this may be for fewer commands
		...
		if (activate_file_map(args) !=0){
		    // the above may actually just wait until it opens
		    // due to how locking works
		    std::cout << "File in use - client" << std::endl;
		    std::cout << "Will initiate thread auto-termination after creation" << std::endl;
		...
		if (error_terminate){
		    send_msg(n_sockfd, "terminate " + reply, false);
		}
	    }
        }
...
void get_thread(int old_fd, std::string thread_id, std::string reply){
    ...
    int t_sockfd = old_fd;
    get(t_sockfd, thread_buffer, reply);
