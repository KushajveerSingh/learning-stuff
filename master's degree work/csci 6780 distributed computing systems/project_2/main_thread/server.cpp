...
int main (int argc, char** argv) {
    ...
    */
	...
	if (get_active_server_threads() < MAX_CLIENTS){
	    ...
	    activate_server_map(new_fd, n_sockfd, &their_addr);// deactivation at end of thread loop
	    
            std::thread client_connection(server_thread, new_fd, t_sockfd);
...
int server_thread(int new_fd, int t_sockfd){//TODO: decomment through while loop
    ...
    while (1) {
        ...
        // delete
    	    ...
    	    if (args.size() == 0)
    	        ...
    	        if (activate_file_map((directory + args).c_str()) != 0){
    	    	    send_msg(new_fd, "file in use, please try again later");
    	        }
    	        else{		    
    	    	    if (remove(args.c_str()) == -1)
    	    	        ...
    	    	        send_msg(new_fd);
    	    	    deactivate_file_map((directory+args).c_str());
    	        }
    	    }
        }
