...
std::unordered_map<int, struct sockaddr_storage *> server_map;//TODO: move to server.hpp
std::mutex server_map_m; //map and mutex of all client threads

class Server_Safety{//TODO: move to server.hpp
...
int get_active_server_threads(){//TODO: move to server.hpp
    std::lock_guard<std::mutex> lock(server_map_m);
    return server_map.size();
}

int activate_server_map(int new_fd, int n_sockfd, struct sockaddr_storage *their_addr){//TODO: move to server.hpp
...
int deactivate_server_map(int new_fd){//TODO: move to server.hpp
    std::lock_guard<std::mutex> lock(server_map_m);
    int r = 0;
    if (server_map.find(new_fd) != server_map.end()){
	r = -1;
