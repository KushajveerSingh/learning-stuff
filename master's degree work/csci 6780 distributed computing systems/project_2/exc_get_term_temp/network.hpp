...
const int terminate_test = 5;//if at 0, code will ignore
std::chrono::seconds dura_term(terminate_test);
...
void send_file (int fd, std::string path, char *msg, bool server = true) {
    ...
    */
    ...
    check_error = recv_msg(fd, msg, server);
    memset(msg, '\0', strlen(msg));
    if (check_error != "NO_MESSAGE"){
	std::cout << "after numbytes sent" << check_error << std::endl;
    }
	...
	if (terminate_test > 0){
	    std::this_thread::sleep_for(dura_term);
	}
	...
	if (not server){
	    ...
	    status = reply[0];
	    if (reply.length()  != 1){
	        std::cout << "after status received:" << reply << std::endl;
            }
	}
	else{
	    ...
	    reply = recv_msg(fd, msg, server);
            if (reply != "NO_MESSAGE"){
	       std::cout << "after status null recieved:" << reply << std::endl;
            }
	}
	memset(msg, '\0', strlen(msg));
    }
    ...
    if (status != 'A'){
	std::cout << "non-active status termination: " << status << std::endl;
	if (status == 'T'){
	    reply = "TERMINATE";
	}
	else{
    ...
    //if (not server){
    //    std::cout << "waiting for final reply" << std::endl;
    //    reply = recv_msg(fd, msg, server);
...
void recv_and_write_file (int fd, std::string path, char *msg, bool print_msg = false, bool server = true) {
    ...
    */
    ...
    if (server){
	file_contents = recv_msg(fd, msg, server);
	if (file_contents != "NO_MESSAGE")
	    std::cout << "reply eaten after loop: " << file_contents << std::endl;
    }
    ...
    std::string closing = "File transfer complete";
    if (print_msg)
