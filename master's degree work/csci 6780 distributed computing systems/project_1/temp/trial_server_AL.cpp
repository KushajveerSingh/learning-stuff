...
int main (int argc, char** argv) {
    ...
    */
    ...
    while (1) { // keep listening for connections
		...
		case 1:
		    // get
		    if (send_file_stream(command_args, new_fd, 1) !=0){
			cout << "warning send_file return was not 0" << endl;
		    }
		    break;
		    //
		    //move_file();
		...
		case 4:
			...
			reply += ": " + command + " " + command_args;
			chdir(command_args.c_str());
			send_msg(new_fd, reply);
		    }
		    break;
		...
		case 6:
			...
			reply += ": " + command + " " + command_args;
			mkdir(command_args.c_str(), EACCES);
			send_msg(new_fd, reply);
		    }
		    break;
